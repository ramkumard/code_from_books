...
class Proxy
  def initialize(target_object)
      # initialize the object class to proxy
    @object = target_object
      # ADD MORE CODE HERE
      # initialize the array of messages.
      # tracking and logging purpose
    @messages = Array.new
  end
  ...
  attr_reader :messages
    ...
    # WRITE CODE HERE
    # http://www.ruby-doc.org/core-1.9.3/BasicObject.html#method-i-method_missing
  def method_missing(method_name, *args, &block)
    if @object.respond_to?(method_name) then
        # track each method_name called that target object can respond to.
      @messages.push(method_name)
        # call the method
        # puts("proxing #{@object} - calling #{method_name}")
      @object.send(method_name, *args, &block)
    else
       # all other cases: default behavior (raise NoMethodError)
  ...
  def number_of_times_called(method_name)
    @messages.select {|method_called| method_called == method_name}.count
  end
...
class AboutProxyObjectProject < EdgeCase::Koan
  ...
  def test_proxy_records_messages_sent_to_tv
    ...
    # the messages method want just keep track of
    # the method called.
    # It's a Proxy feature, not a Television feature
