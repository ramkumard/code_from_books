class Tactics
  ...
  def self.all_possible_moves
    # Replicate the possibilities for a single row over each row and column of the grid.
    (0..3).inject([]) do |moves, row|
      [0b1000, 0b0100, 0b0010, 0b0001, 0b1100, 0b0110, 0b0011, 0b1110, 0b0111, 0b1111].each do |bits|
        move = bits << 4 * row
  ...
  def self.transpose(board)
    (0..15).inject(0) { |xboard, i| q,r = i.divmod(4); xboard |= board[i] << q + r*4 }
  end
end
