...
USAGE = "countdown.rb [options] target (numbers...)
    ...
    --integral -i   Report only solutions that create only integral subsolutions
                    The integral version is a lot faster than the fractional version. (For six cards ~ factor two)

                    Default is to allow all solutions.
    --quiet    -q   Don't report partitial solutions
    --memoized -m   Use memoization to speed up the search. This will eat lots of ram and if there is no more ram,
                    speed will even be worse than without memoization.
    --recursive -r  Use simple recursive brute force search. Will use only a constant amount of memory, but calculates
                    a lot of results multiple times."
...
# Systematically create all terms over all subsets the set of numbers in source, and find the one
# that is closest to target.
#
# Returns the solution that is closest to the target.
#
# If a block is given, calls the block each time a better or equal solution is found.
#
