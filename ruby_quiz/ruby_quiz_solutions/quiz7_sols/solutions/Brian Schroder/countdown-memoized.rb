...
module Memoized
  module Fraction
    # Call the given block for each term that can be constructed over a set of numbers.
    #
    # Recursive implementation, that calls a block each time a new term has been stitched together.
    #
    # Uses memoization to speed up creation of solutions, and returns each solution only once.
    #
    # This may be a bit slow, because it zips up through the whole callstack each time a new term
    # is created.
    #
    # It was a bit more compact when I used arrays to iterate, but on the other hand now I'm pruning
    # some of the more stupid operations.
          ...
          each_term_over(p1, memo, &block).each do | op1 |
            each_term_over(p2, memo, &block).each do | op2 |
              ...
              if op1.value != 0.0
                ...
                if op1.value != 1.0
                  result << Term.new(op2, op1, :'/')
                  result << Term.new(op1, op2, :*) if op2.value != 0.0 and op2.value != 1.0
                end
              end
            end
          end
  ...
  module Integral
    ...
    #
    # Here I explicitly coded the operators, because there is not much redundance.
    #
    # This may be a bit slow, because it zips up through the whole callstack each time a new term
    # is created.
