...
class PackagingPreProcessor
  ...
  openStack=[]
    ...
    aMsg.each_with_index do |c,i|
      ...
      # Push opening packaging type onto the stack along with the index
      if opening?(c)
        type=getPackagingType(c)
        openStack.push([type,i])
      end
      ...
      if closing?(c)
        ...
        (expectedPackaging,iOpeningPos)=openStack.pop        
        ...
        if (currentPackagingTag != expectedPackaging)
          ...
          #     or
          #     There had been a subsequent position opened that needs to
          #     be closed first - eg, [{B] - mismatch of expected brackets
          #
          ...
          if (aMsg.select { |c| expectedPackaging.consistsOf?(c) }.length % 2 != 0)
            ...
            # Insertion point is directly after where the expected bracket
            # was opened           
                            
            msg.final = msg.original[0..(iOpeningPos)] + currentPackagingTag.opening + msg.original[(iOpeningPos+1)..-1]
            return
          end                   
        end                
      end      
    ...
    if openStack.length!=0
      ...
      # EG, [{B}
      (expectedPackaging,iOpeningPos) = openStack.pop
      msg.final=msg.original + expectedPackaging.closing      
      return
    end
...
ARGF.each do |line|
  ...
  #print "#{msg.original} -> #{msg.final} (valid=#{msg.valid?})\n"
